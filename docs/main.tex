...
\end{minted}
\caption{Adding the Map to the GUI}\label{cs:adjustParameters}
\end{listing}
...
\csvloop{
    ...
    late after line=\\\hline,
    after reading={
        \caption{Testing Table for Code Snippets \ref{cs:initialiseDrawingMap} \& \ref{cs:adjustParameters}}
        \end{longtable}
    }
}
\end{center}
...
\end{minted}
\pil{app.py / def processPrediction}
\begin{minted}[linenos,breaklines,frame=single,firstnumber=42]{python}
    # calculate area
...
Figure \ref{fig:ss12.5} shows the prediction for Oxford, whici is pretty close.
