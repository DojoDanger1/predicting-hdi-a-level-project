...
Here, I am defining the \pil{def calcArea} function, it takes in the \pil{bounding_coords} as a parameter. The first thing we need to check is if the first and last elemenents of that list are the same. If they are, then remove the last one, as we don't want to be double-counting a particular vertex.
...
\stepcounter{parts}
\subsection{Part \theparts{} -- Training the Neural Network}
...
\subsection{Storing Data \& Encryption}
\dots

\subsection{Creating an Account \& Logging In}
\dots

\subsection{Saving Regions}
\dots
...
\subsection{Analysis of how much Each Factor affects the Others}
\dots

\subsection{Manually Entering Factors}
\dots

\subsection{Comparison to Similar Regions}
\dots

\subsection{Improved Algorithm for Finding an Optimal Place for a New Building}
\dots
