...
I also have a fully functioning authentication system, where the user can create and log into an account.

\section{Milestone 5 -- Final User Interface}
\subsection{Success Criteria}
By the end of this milestone, I hope to have a full user interface, with all of the desired featured. At the end, I aim to deploy the app on Hugging Face. 
\begin{center}
\csvloop{
    file=data/successcriteria.csv,
    no head,
    column count=5,
    column names={1=\scID, 2=\scCriterion, 3=\scJustification, 4=\scSuccess, 5=\scEvaluation},
    filter ifthen={\equal{\scID}{$S_{14}$}\OR\equal{\scID}{$S_{15}$}\OR\equal{\scID}{$S_{16}$}\OR\equal{\scID}{$S_{17}$}\OR\equal{\scID}{$S_{18}$}\OR\equal{\scID}{$S_{19}$}\OR\equal{\scID}{$S_{20}$}\OR\equal{\scID}{\textbf{No.}}},
    before reading={
        \begin{longtable}{|m{1cm}|m{5cm}|m{8cm}|H H}
        \hline
    },
...
\csvloop{
    ...
    late after line=\\\hline,
    after reading={
        \caption{$T_{16}$ Testing Table (for Code Snippets \ref{cs:allObjectsInConsideration}, \ref{cs:iteratingOverFactors}, \ref{cs:calcShortestDistances} \& \ref{cs:returnOptimalPosition})}
        \end{longtable}
    }
}
\end{center}
...
\caption{Markdown Test Successul}\label{fig:ss22.17}
\end{figure}

Figure \ref{fig:ss22.16} shows the top part of the appearance of the Help Tab, now that I have written the full text. I will not include it here, as it is quite long, but it will be included in the Final Code Section (Appendix \ref{sec:finalCode}).
...
\subsection{Testing for Function}
\dots{}

\subsection{Testing for Robustness}
\dots{}

\subsection{Testing for Usability}
\dots{}

\section{Evaluation of the Coded Solution}
\dots{}
